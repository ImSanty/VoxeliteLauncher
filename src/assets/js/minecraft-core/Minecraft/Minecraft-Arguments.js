...
class MinecraftArguments {
     ...
     */
    ProcessDefaultUserJVMArgs(versionJson, existingArgs) {
        ...
        for (const argEntry of defaultUserJVM) {
            ...
            if (argEntry.rules && !this.EvaluateRules(argEntry.rules))
                continue;
            // Extract values
            let values = [];
            if (typeof argEntry.value === 'string')
                values = [argEntry.value];
            else if (Array.isArray(argEntry.value))
                values = argEntry.value;
