...
class Splash {
  ...
  async startAnimation() {
    let splashes = [
      { message: 'Estoy vivo!', author: 'ImSanty' },
      { message: 'Hola desde el codigo.', author: 'ImSanty' }
    ];
  ...
  async checkUpdate() {
    ...
    ipcRenderer.on('update-not-available', () => {
      console.error('No hay actualizaciones disponibles');
  ...
  async dowloadUpdate() {
    ...
    this.setStatus(
      `Actualizacion disponible!<br><div class="download-update">Descargando</div>`
    );
    document.querySelector('.download-update').addEventListener('click', () => {
      shell.openExternal(latest.browser_download_url);
      return this.shutdown('Descarga en proceso...');
    });
  }
  ...
  async maintenanceCheck() {
    config
      ...
      .catch((e) => {
        console.error(e);
        return this.shutdown(
          'No se detecto una conexion a internet,<br>Intente de nuevo mas tarde.'
        );
      });
  }

  startLauncher() {
    this.setStatus(`Inciando el launcher`);
    ipcRenderer.send('main-window-open');
    ipcRenderer.send('update-window-close');
  }

  shutdown(text) {
    this.setStatus(`${text}<br>Finalizando en 5s`);
    let i = 4;
    setInterval(() => {
      this.setStatus(`${text}<br>Finalizando en ${i--}s`);
